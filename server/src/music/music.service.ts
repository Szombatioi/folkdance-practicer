...
export class MusicService {
  ...
  async create(createMusicDto: CreateMusicDto) {
    ...
    music.lyrics = [];
    music.musicSources = [];
    console.log(musicLyricsIds);
    for (const id of musicLyricsIds) {
      const lyric = await this.musicLyricRepository.findOne({ where: { id } });
      if (!lyric)
        throw new NotFoundException(`Music lyric not found with id #${id}`);
      music.lyrics.push(lyric);
    }

    for (const id of musicSourceIds) {
      const source = await this.musicSourceService.findOne(id);
      if (!source)
        throw new NotFoundException(`Music source not found with id #${id}`);
  ...
  async findOne(id: number) {
    ...
    if (!music)
      throw new NotFoundException(`Music did not found with id #${id}`);
  ...
  async update(id: number, updateMusicDto: UpdateMusicDto) {
    ...
    if (musicLyricsIds)
      musicLyricsIds.forEach(async (id) => {
        const lyric = await this.musicLyricRepository.findOne({
          where: { id },
        });
        if (!lyric)
          throw new NotFoundException(
            `Music lyric did not found with id #${id}`
          );
        music.lyrics.push(lyric);
      });
    ...
    if (delMusicLyricIds)
      delMusicLyricIds.forEach(async (id) => {
        const lyric = await this.musicLyricRepository.findOne({
          where: { id },
        });
        if (lyric) {
    ...
    if (delMusicSourceIds)
      delMusicSourceIds.forEach(async (id) => {
        const source = await this.musicSourceService.findOne(id);
        if (source) {
