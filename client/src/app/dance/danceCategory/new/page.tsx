...
export default function NewDanceCategoryPage() {
    ...
    const handleCreateButton = async () => {
        ...
        const response = await axios.post("http://localhost:3001/dance-categories", payload)
        if (response.status >= 200 && response.status < 300) {
        ...
        <>
            <Box margin={4} justifySelf="center" alignSelf="center" >
                <Paper elevation={3} sx={{ alignItems: "center", paddingY: 4, paddingX: 8, flexDirection: "column", display: "flex", gap: 2, flexGrow: 0 }}>
                    <Typography variant="h4" align="center">Új tánc kategória felvétele</Typography>
                    <TextField onChange={(e) => setDcName(e.target.value)} value={dcName} variant="outlined" placeholder="Kategória neve"></TextField>
                    <Button onClick={() => handleCreateButton()} variant="contained">Létrehozás</Button>
                </Paper>
            </Box>
            <Box margin={4} justifySelf="center" alignSelf="center" >
                <Paper elevation={3} sx={{ alignItems: "center", paddingY: 4, paddingX: 8, flexDirection: "column", display: "flex", gap: 2, flexGrow: 0 }}>
                    <Typography variant="h4" align="center">Tánc kategóriák lekérdezése</Typography>
                    <Button onClick={() => fetchDanceCategories()} variant="contained">Lekérdezés</Button>
                    {danceCategories.length > 0 && (
                        <TableContainer style={{ width: "300px", border: "1px solid grey", borderRadius: "8px" }}>
                            <Table>
                                <TableHead>
                                    <TableRow>
                                        <TableCell colSpan={2} align="center">Kategória neve</TableCell>
                                    </TableRow>
                                </TableHead>
                                <TableBody>
                                    {danceCategories.map((figureType, index) => (
                                        <TableRow key={index}>
